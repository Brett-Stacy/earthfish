...
create_casal_file_pop <- function(params, casal_path, skel_csl, csl) {
  ...
  # extract casal parameters from skeleton file
	...
	if (params$by_sex==0) {
		...
		casalpop[["size_at_age"]]$by_length <- "True"
	} else {
	...
	if (sum(ttag) > 0) {
		...
		for (ff in 1:nrow(tag_names)) {
			...
			if(sum(params$catch[[tfish]][,tyear,,]) > 0) {
				...
				if (datass$tag_sampling_type == "size") casalpop[[tagN]]$class_mins <- params$class_mins # BS 1/10/19: condition on if using size for tags
				if (params$by_sex == 0) {
