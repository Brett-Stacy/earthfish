...
ogive <- function(type, ages, params){
  ...
  ## Ensure if ramp type, params do not have decimal points (this leads to periodic SSB estimates for some reason)
  if(type == "ramp" & any(!as.integer(unlist(params)) == unlist(params)))
    stop(paste0("supplied ramp params must be integers")) # Ramp type ogive doesn't like non-integer parameter imputs
  ...
  switch(type,
         ...
         dbramp = {
           ...
           down <- params[["high0"]]-params[["high1"]] + 1
           res <- c(rep(0,params[["low0"]]-ages[1]),round(seq(0,1,length.out=up),4),rep(1,(params[["high1"]]-params[["low1"]]-1)),
                    round(seq(1,0,length.out=down),4),rep(0,ages[length(ages)]-params[["high0"]]))},
         provide = res <- params)
  ## return the ogive
  res
}
